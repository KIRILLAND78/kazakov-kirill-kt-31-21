...
namespace kazakov_kirill_kt_31_21.Interfaces.ProfessorInterfaces
{
    ...
    public class ProfessorService : IProfessorService
    {
        ...
        public Task<List<Professor>> GetProfessorsByFilterAsync(ProfessorGroupFilter filter, CancellationToken cancellationToken = default)
        {
            ...
            if (filter.FacultyId != null) professors.Where(x=>x.FacultyId==filter.FacultyId);
            if (filter.PostId != null) professors.Where(x => x.PostId == filter.PostId);
            if (filter.RankId != null) professors.Where(x => x.RankId == filter.RankId);
